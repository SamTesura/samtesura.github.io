...
// ============================================================================
// RIOT API INTEGRATION - Live Game Loader
// ============================================================================

const RIOT_CONFIG = {
  SUMMONER_API: 'https://{region}.api.riotgames.com/lol/summoner/v4/summoners/by-name/{summonerName}',
  ACTIVE_GAME_API: 'https://{region}.api.riotgames.com/lol/spectator/v5/active-games/by-summoner/{encryptedSummonerId}',
  API_KEY_STORAGE: 'riot_api_key',
  CORS_PROXY_STORAGE: 'cors_proxy'
};

function setupLiveGameListeners() {
  const toggleBtn = document.getElementById('toggleLiveGame');
  const liveGameContent = document.getElementById('liveGameContent');
  const loadGameBtn = document.getElementById('loadGameBtn');
  const apiKeyInput = document.getElementById('riotApiKey');
  const corsProxyInput = document.getElementById('corsProxy');

  // Load saved API key and CORS proxy from localStorage
  const savedApiKey = localStorage.getItem(RIOT_CONFIG.API_KEY_STORAGE);
  if (savedApiKey) {
    apiKeyInput.value = savedApiKey;
  }

  const savedCorsProxy = localStorage.getItem(RIOT_CONFIG.CORS_PROXY_STORAGE);
  if (savedCorsProxy) {
    corsProxyInput.value = savedCorsProxy;
  } else {
    // Set default CORS proxy
    corsProxyInput.value = 'https://corsproxy.io/?';
  }

  // Toggle live game section
  toggleBtn.addEventListener('click', () => {
    const isHidden = liveGameContent.classList.contains('hidden');
    if (isHidden) {
      liveGameContent.classList.remove('hidden');
      toggleBtn.textContent = 'Hide';
    } else {
      liveGameContent.classList.add('hidden');
  ...
  apiKeyInput.addEventListener('change', () => {
    ...
    if (apiKey) {
      localStorage.setItem(RIOT_CONFIG.API_KEY_STORAGE, apiKey);
    }
  });

  // Save CORS proxy when user types
  corsProxyInput.addEventListener('change', () => {
    const corsProxy = corsProxyInput.value.trim();
    localStorage.setItem(RIOT_CONFIG.CORS_PROXY_STORAGE, corsProxy);
  });

  // Load game button
  loadGameBtn.addEventListener('click', loadFromLiveGame);
}

async function loadFromLiveGame() {
  const apiKeyInput = document.getElementById('riotApiKey');
  ...
  // Validate inputs
  if (!apiKey) {
    showLiveGameStatus('error', 'Please enter your Riot API key. Get one from https://developer.riotgames.com/');
    return;
  }

  if (!summonerName) {
    showLiveGameStatus('error', 'Please enter a summoner name.');
    return;
  }
  ...
  showLiveGameStatus('info', 'Fetching summoner data...');

  try {
    // Step 1: Get summoner data
    const summonerData = await fetchSummonerByName(region, summonerName, apiKey, corsProxy);

    if (!summonerData) {
      throw new Error('Summoner not found. Please check the summoner name and region.');
    }

    showLiveGameStatus('info', `Found summoner: ${summonerData.name}. Fetching active game...`);

    // Step 2: Get active game
    const gameData = await fetchActiveGame(region, summonerData.puuid, apiKey, corsProxy);

    if (!gameData) {
      throw new Error('No active game found. The summoner must be in a live game (champion select or in-game).');
    }

    showLiveGameStatus('info', 'Active game found! Loading champions...');

    // Step 3: Populate champions from game data
    await populateChampionsFromGame(gameData, summonerData.puuid);

    showLiveGameStatus('success', `✓ Successfully loaded ${gameData.participants.length} champions from live game!`);
    ...
    showLiveGameStatus('error', error.message || 'Failed to load live game. Please check your API key and try again.');
  } finally {
    loadGameBtn.disabled = false;
    loadGameBtn.textContent = '🔄 Load Live Game';
  }
}

async function fetchSummonerByName(region, summonerName, apiKey, corsProxy) {
  // Riot API v5 uses puuid, so we need to use the newer endpoint
  let url = `https://${region}.api.riotgames.com/lol/summoner/v4/summoners/by-name/${encodeURIComponent(summonerName)}`;

  // Apply CORS proxy if provided
  if (corsProxy) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch summoner: ${response.status} ${response.statusText}`);
  }

  return await response.json();
}

async function fetchActiveGame(region, puuid, apiKey, corsProxy) {
  let url = `https://${region}.api.riotgames.com/lol/spectator/v5/active-games/by-summoner/${encodeURIComponent(puuid)}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch active game: ${response.status} ${response.statusText}`);
  }

  return await response.json();
}

async function populateChampionsFromGame(gameData, playerPuuid) {
  // Find the player's participant data
  const playerParticipant = gameData.participants.find(p => p.puuid === playerPuuid);

  if (!playerParticipant) {
    throw new Error('Could not find player in game data.');
  }

  // Get champion data for the player
  const playerChampionId = playerParticipant.championId;
  const playerChampion = Object.values(state.champions).find(c => parseInt(c.key) === playerChampionId);

  if (!playerChampion) {
    throw new Error(`Could not find champion data for champion ID ${playerChampionId}.`);
  }

  // Check if player is playing ADC
  const adcIds = ADC_LIST.getAllADCs();
  const isPlayingADC = adcIds.includes(playerChampion.id);

  if (isPlayingADC) {
    // Auto-select the player's ADC
    selectADC(playerChampion);
  } else {
    // Show a message that they need to select their ADC manually
    showLiveGameStatus('info', `You're playing ${playerChampion.name}. Please select your ADC manually above.`);
  }

  // Determine player's team (100 = Blue, 200 = Red)
  const playerTeamId = playerParticipant.teamId;

  // Separate participants into allies and enemies
  const allies = gameData.participants.filter(p =>
    p.teamId === playerTeamId && p.puuid !== playerPuuid
  );
  const enemies = gameData.participants.filter(p =>
  ...
  // Get input elements
  const enemyInputs = document.querySelectorAll('#enemyInputs input');
  const allyInputs = document.querySelectorAll('#allyInputs input');

  // Populate enemy champions
  enemies.forEach((participant, index) => {
    if (index < enemyInputs.length) {
      const championId = participant.championId;
      const champion = Object.values(state.champions).find(c => parseInt(c.key) === championId);

      if (champion) {
        enemyInputs[index].value = champion.name;
        state.enemies[index] = champion;
        updateDeleteButtonVisibility(enemyInputs[index]);
      }
    }
  });

  // Populate ally champions
  allies.forEach((participant, index) => {
    if (index < allyInputs.length) {
      const championId = participant.championId;
      const champion = Object.values(state.champions).find(c => parseInt(c.key) === championId);

      if (champion) {
        allyInputs[index].value = champion.name;
        state.allies[index] = champion;
        updateDeleteButtonVisibility(allyInputs[index]);
      }
    }
  });
...
function showLiveGameStatus(type, message) {
  const statusDiv = document.getElementById('liveGameStatus');
  statusDiv.className = `live-game-status visible ${type}`;
